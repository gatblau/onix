...
func (b *ComposeBuilder) Build() ([]DeploymentRsx, error) {
	...
	rsx = append(rsx, deployScript, b.buildFile())
	return rsx, nil
}
...
# deploy.sh
# deploy the %[1]s application
# autogenerated by Onix Artisan on %[2]s
`, b.manifest.Name, time.Now().UTC()))
...
func (b ComposeBuilder) buildFile() DeploymentRsx {
	buildYAML := `---
# build.yaml for %[1]s application
# autogenerated using Onix Artisan on %[2]s
functions:
  - name: deploy
    description: deploys the %[1]s application using docker-compose
    run:
      - sh deploy.sh
      - art runc init -n %[3]s
        
  - name: destroy
    description: destroys all containers of the XXX application 
    run:
      - sh destroy.sh
        
  - name: init
    description: initialises services after containers start
    runtime: dbman
    run:
      - sh init.sh
...
`
	return DeploymentRsx{
		Name:    "build.yaml",
		Content: []byte(fmt.Sprintf(buildYAML, b.manifest.Name, time.Now().UTC(), b.network())),
		Type:    BuildFile,
	}
}
