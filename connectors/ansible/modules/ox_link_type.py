...
def deleteLinkType(module):
    ...
    stream = open_url(link_type_uri, method="DELETE", headers=getHeaders(access_token))
...
def createOrUpdateLinkType(module):
    ...
    # put the payload to the cmdb service
    stream = open_url(link_type_uri, method="PUT", data=payloadStr, headers=getHeaders(access_token))
